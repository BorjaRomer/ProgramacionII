...
public class JDialog_MostrarCamiones extends JDialog implements ActionListener{
	...
	private ArrayList<itfProperty> camiones;
	private ArrayList<itfProperty> camiones2;
	...
	private ModeloTablaCamiones tcm;
	...
	public JDialog_MostrarCamiones(clsGestor _objGestor, String _operario) {
		...
		JLabel lblInTitulo = new JLabel("CAMION SELECCIONADO:");
		...
		comboMarc.setModel(new DefaultComboBoxModel<String>(new String[] {"Todos","Mercedes", "Man", "Scania", "Volvo", "Renault", "Daf"}));
		...
		JLabel lblEncontrarCoches = new JLabel("BUSQUEDA DE CAMIONES");
		lblEncontrarCoches.setForeground(Color.BLACK);
	...
	public void actionPerformed(ActionEvent e) {
		...
		case BOTON_VENDER:
			...
			if (fila == -1) {
				...
				JOptionPane.showInternalMessageDialog(null, "No has seleccionado ningun camion");
				...
				try {
					...
					camiones2 = objGestor.DameCamionesv2();
					
					for (itfProperty c : camiones2) {
						if (c.getProperty(PROPIEDAD_NUMBASTIDOR).equals(numbastidor)) {
							listModelo.addElement("------ " + c.getProperty(PROPIEDAD_MARCA) + " ------ " + c.getProperty(PROPIEDAD_MODELO)+ " ------ Tasación Inicial: "+
		...
		case BOTON_ELIMINAR:
			...
			if (fila == -1) {
				...
				if (respuesta == 0) {
					try {
						objGestor.EliminarcamionBD(numbastidor);
		...
		case BOTON_INTERNAL_VENDER:
			...
			try {
				...
				try {
					...
					internal_venta.dispose();
					objGestor.modificarEstadoCamion(numbastidor);
	...
	public void CrearTabla() {
		...
		tcm = new ModeloTablaCamiones(camiones, tipocamion);		
		...
		tcm.setData(camiones, tipocamion);
